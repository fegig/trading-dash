...
function AssetsList({ userCoins }: { userCoins: UserCoinsProps[] }) {
    ...
    const handleModalType = (type: 'receive' | 'send' | 'swap', selectedCoin: UserCoinsProps) => {
        setModalType(type);
        setIsModalOpen(true);
    ...
    return (<> {
            ...
            <div className="gradient-background p-4 rounded-lg flex flex-col  space-y-4 min-w-sm max-md:min-w-2xs" key={coin.walletId}>
                ...
                <div className="flex justify-between items-center gap-2 mt-2">
                    <button
                        ...
                        className="flex-1 rounded-full gradient-background !p-2 !py-1 hover:bg-neutral-800/50 smooth flex items-center justify-center space-x-1 text-xs">
                        <i className="fi fi-rr-arrow-down text-neutral-500"></i>
                        <span>Receive</span>
                    </button>
                    <button onClick={() => handleModalType('send', coin )}
                        className="flex-1 rounded-full gradient-background !p-2 !py-1 hover:bg-neutral-800/50 smooth flex items-center justify-center space-x-1 text-xs">
        ...
        <Modal
                    ...
                    title={``}
                >
                    {modalType === 'send' && <Send coin={selectedCoin} />}
                    {modalType === 'receive' && <Receive coin={selectedCoin} />}
                    {modalType === 'swap' && <Swap coin={selectedCoin} />}
                </Modal>
        )}
                </>
    )
